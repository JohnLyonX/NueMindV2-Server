    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['edu:coursesExam:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['edu:coursesExam:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['edu:coursesExam:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['edu:coursesExam:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="coursesExamList" @selection-change="handleSelectionChange">
        ...
        <el-form-item label="课程名称" prop="coursesName">
          ...
          >
            <el-option
              ...
              :label="course.name"
              :value="course.name"
              @click.native="handleCourseSelect(course)"
            />
          </el-select>
        </el-form-item>
...
<script>
import { listCoursesExam, getCoursesExam, delCoursesExam, addCoursesExam, updateCoursesExam } from "@/api/edu/coursesExam";
...
export default {
  ...
  methods: {
    ...
    handleCourseSelect(course) {
      this.form.coursesId = course.id;
      this.form.coursesName = course.name;
    },
  // 获取部门树数据
  getDeptTree() {
  ...
  findDeptById(id, list = this.deptOptions) {
    ...
    handleUpdate(row) {
      ...
      getCoursesExam(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCoursesExam(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCoursesExam(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('edu/coursesExam/export', {
        ...this.queryParams
      }, `coursesExam_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
